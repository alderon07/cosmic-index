...
export interface ExoplanetsPageClientProps {
  initialData: PaginatedResult<ExoplanetData> | null;
  initialError: string | null;
  initialFetchKey: string;
}

function buildExoplanetFetchKey(input: {
  searchQuery: string;
  ...
  if (input.filters.discoveryMethod) params.set("discoveryMethod", input.filters.discoveryMethod);
  if (input.filters.year) params.set("year", input.filters.year.toString());
  if (input.filters.hasRadius) params.set("hasRadius", "true");
  if (input.filters.hasMass) params.set("hasMass", "true");
  if (input.filters.sizeCategory) params.set("sizeCategory", input.filters.sizeCategory);
  if (input.filters.habitable) params.set("habitable", "true");
  if (input.filters.facility) params.set("facility", input.filters.facility);
...
export function ExoplanetsPageClient({
  initialData,
  initialError,
  initialFetchKey,
}: ExoplanetsPageClientProps) {
  ...
  const [data, setData] = useState<PaginatedResult<ExoplanetData> | null>(initialData);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const fetchKey = buildExoplanetFetchKey({ searchQuery, filters, page, limit });
      const result = await apiFetchPaginated<ExoplanetData>(`/exoplanets?${fetchKey}`);
      setData(result);
    } catch (err) {
  ...
  const currentFetchKey = useMemo(
    () => buildExoplanetFetchKey({ searchQuery, filters, page, limit }),
    [searchQuery, filters, page, limit]
  );
