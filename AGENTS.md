...
- Avoid `any`; use `unknown` or define precise shapes instead.
- Avoid `React.FC`; use standard function declarations and explicitly typed props.
- Use specific event types such as `React.ChangeEvent<HTMLInputElement>` or
  `React.FormEvent` instead of broad event parameters.
